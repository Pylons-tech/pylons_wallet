...
internal fun createCookbook(msg: MessageData): Response {
    ...
    require (msg.ints.containsKey(Keys.COSTPERBLOCK)) { throw BadMessageException("createCookbook", Keys.COSTPERBLOCK, "Int") }
    val tx = Core.createCookbook(msg.strings[Keys.NAME]!!, msg.strings["devel"]!!, msg.strings["desc"]!!,
            msg.strings["version"]!!, msg.strings["supportEmail"]!!, msg.ints["level"]!!.toLong(), msg.ints["costPerBlock"]!!.toLong())
...
fun Core.createCookbook (name : String, devel : String, desc : String, version : String,
        ...
        engine.createCookbook(name, devel, desc, version, supportEmail, level, costPerBlock).submit()