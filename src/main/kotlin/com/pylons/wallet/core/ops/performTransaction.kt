...
fun performTransaction (msg : MessageData) : Response {
    ...
    if (!msg.strings.containsKey(Keys.OTHER_ADDRESS)) return generateErrorMessageData(Error.MALFORMED_TX, "Did not provide an ID for remote profile involved in transaction.")
    if (bufferForeignProfile(msg.strings[Keys.OTHER_ADDRESS]!!) == null) return generateErrorMessageData(Error.FOREIGN_PROFILE_DOES_NOT_EXIST, "No profile with provided ID exists.")
...
fun Core.performTx (txDesc : TransactionDescription) : Transaction = Core.engine.commitTx(Transaction.build(txDesc)!!)