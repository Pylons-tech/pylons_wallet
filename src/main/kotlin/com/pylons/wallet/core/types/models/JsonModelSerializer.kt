...
object JsonModelSerializer {
    ...
    private fun handleComplexValues (prop : KProperty1<out Any, Any?>, value : Any?, p0: JsonWriter) {
        when {
            ...
            Regex("kotlin.List<.*>").matches(prop.returnType.toString()) -> handleLists(prop, value, p0)
            // TODO: maps?
    ...
    private fun handleLists (prop : KProperty1<out Any, Any?>, value : Any?, p0: JsonWriter) {
        p0.beginArray()
        when (prop.returnType.toString()) {
            "kotlin.List<kotlin.String>" -> (value as List<String>).forEach { p0.value(protectWhitespace(it)) }
            ...
            else -> (prop as List<*>).forEach { serialize(p0, it) }
        }
        p0.endArray()
    }
}